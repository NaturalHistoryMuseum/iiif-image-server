...
def parse_identifiers(identifiers: str) -> Iterable[Tuple[AbstractProfile, str]]:
    """
    Given a comma separated list of identifiers, split them up and yield unique
    (profile, name) pairs. Each identifier in the string can be either a full identifier
    <profile>:<name> or one using the default profile <name>. Only unique identifiers
    are returned to avoid downloading the same file multiple times for no reason.
...
async def zip_originals(
    identifiers: str, use_original_filenames: bool = True
) -> StreamingResponse:
    """
    Endpoint which streams a zip containing the original versions of the requested
    images. The zip is created on the fly which in theory means it could be unlimited in
    size, however, to try and keep things under control and because of implementation
    shortcomings, a limit is set in the config.

    Any requested identifiers that can't be found are simply not included in the
    downloaded zip. \f

    :param identifiers: a comma separated list of identifiers (<profile name>:<name>)
        and/or just names in which case the default profile is used
    :param use_original_filenames: whether to use the original file names in the zip
        (True, the default) or name the files after the image name
    :return: a StreamingResponse object streaming a dynamically generated zip of the
        requested original files
    """
